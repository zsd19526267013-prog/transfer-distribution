...
func directBuyIndex(c *gin.Context) {
	var products []CodeProduct
	db.Where("is_active = ?", true).Order("sort_order asc, id asc").Find(&products)
	c.HTML(http.StatusOK, "buy_index.html", gin.H{
		"products": products,
	})
}

func directBuyCreate(c *gin.Context) {
	...
	now := time.Now().Unix()
	outTradeNo := fmt.Sprintf("DC%d%d", now, stock.Id)
	...
	money := fmt.Sprintf("%.2f", float64(product.Price)/100)
...
func directBuySuccess(c *gin.Context) {
	id := c.Param("id")
	var order SellOrder
	...
	c.HTML(http.StatusOK, "buy_success.html", gin.H{
		"order":    order,
		"product":  product,
		"codeKey":  codeKey,
	})
...
func directBuyStatus(c *gin.Context) {
	id := c.Param("id")
	var order SellOrder
	if db.First(&order, id).Error != nil {
...
func directBuyQuery(c *gin.Context) {
	c.HTML(http.StatusOK, "buy_query.html", gin.H{})
}

func directBuyQueryLookup(c *gin.Context) {
	q := strings.TrimSpace(c.PostForm("out_trade_no"))
	if q == "" {
		c.HTML(http.StatusOK, "buy_query.html", gin.H{"err": "请输入订单号"})
		return
	}

	var order SellOrder
	if err := db.Where("out_trade_no = ?", q).First(&order).Error; err != nil {
		c.HTML(http.StatusOK, "buy_query.html", gin.H{"err": "订单不存在，请检查订单号"})
		return
	}

	c.Redirect(http.StatusFound, fmt.Sprintf("/buy/success/%d", order.Id))
}
